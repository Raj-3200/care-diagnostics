...
export const getReportByNumber = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
  try {
    const report = await reportService.getReportByNumber(
...
export const getReportByVisit = async (
  req: Request,
  res: Response,
  next: NextFunction,
) => {
...
export const dispatchReport = async (
  ...
  try {
    const authReq = req as AuthenticatedRequest;
    const { notes } = req.body;
    const report = await reportService.dispatchReport(
      req.params.id,
      notes,
      authReq.user!.userId,
    );
...
export const downloadReport = async (
  ...
  try {
    ...
    const visit = report.visit as any;
    const patient = visit?.patient;
    const testOrders = visit?.testOrders ?? [];

    const testResults = testOrders.map((to: any) => ({
    ...
    const pdfData = {
      ...
      approvedBy: report.approvedBy
        ? { firstName: (report.approvedBy as any).firstName, lastName: (report.approvedBy as any).lastName }
        : null,
      notes: report.notes,
